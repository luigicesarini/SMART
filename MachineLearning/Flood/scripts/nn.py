...
print(os.getcwd())
#os.chdir(<SMART>)
# Create an early stopping callback. This callbakc is used to stop the training once
...
for st in sampling_technique:
  ...
  - Adding one by one the soil moisture layer to the 6 rainfall datasets
    """
  for j in np.arange(len(list_comb)-1,len(list_comb)):
    """
    ...
    for z in np.arange(len(list_comb[j])):
      ...
      for J in np.arange(8,9):#num_hidden:
        ...
        for n,a in enumerate(hidden_act):
          ...
          #write the model
          model = keras.Model(inputs=inputs, outputs=outputs, name="try_fucntional")	
          ...
          for k in np.arange(len(prob_v)):
              ...
              metrics = {
                  # Configuration provides and indication of the model's configuration trained. it reports:
